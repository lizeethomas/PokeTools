...
namespace PokéTools.Tools
{
    public static class PokeTools
    {
        ...
        public static void EnrichTsvWithIdentifiers()
        {
            ...
            if (lines.Count == 0)
                throw new Exception("Fichier TSV vide");

            var headers = lines[0].Split('\t').ToList();
            headers.Add("PokeApiIdentifier");
            ...
            for (int i = 1; i < lines.Count; i++)
            {
                ...
                string form = parts[3].Trim();
                string apiId = BuildIdentifier(name, form);
                string enrichedLine = lines[i] + "\t" + apiId;
